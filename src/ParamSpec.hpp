...
public:
  ...
  {
    return a.name == b.name && a.type == b.type && a.lower_bound == b.lower_bound && a.upper_bound == b.upper_bound;
  }
...
public:
  ...
  CatParamSpec(std::string name, const std::vector<std::string> &cats)
    : ParamSpec(std::move(name),
      ParamSpec::Type::CAT,
      0.0F,
      static_cast<float>(cats.size()) - 1e-6F),
      cats(cats) {}
    ...
    -> float override
  {
    ...
    if (r != cats.end()) {
      return static_cast<float>(std::distance(cats.begin(), r));
    }
    return -1.0;
  }
};
