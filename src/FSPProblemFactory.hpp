...
public:
  static void init(const std::string &new_data_folder)
  {
    FSPProblemFactory::data_folder = new_data_folder;
    loadLowerBoundsData();
  }

  static auto names() -> std::vector<std::string>
  {
    return { "problem", "type", "instance", "objective", "budget", "stopping_criterion" };
  }
  ...
  static void loadLowerBoundsData()
  {
    ...
    if (!fl.good()) {
      throw std::runtime_error("lower bounds data file not found in " + lowerBoundsFile());
    }
  ...
  {
    auto l = std::find_if(
      lower_bounds_data.begin(), lower_bounds_data.end(), [&](const std::unordered_map<std::string, std::string> &line) {
        return line.at("instance") == instance && line.at("objective") == objective;
      });
    ...
    -> FSPProblem
  {
    ...
    return FSPProblem(inst, type, objective, budget, stoppingCriterion, lowerBound);
  }
};
